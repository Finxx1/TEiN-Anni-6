...
const vec4 light_color = vec4(1.0, 1.0, 1.0, 2.0); //r, g, b, a. the gradient light that is above (and on top) of the water
const vec4 glow_color = vec4(1.0, 1.0, 1.0, 3.0); //alpha is unused here. the color that white parts glow when near water
const float glow_strength = 48;                 //how strong the glow effect is
const float glow_threshold = .1;               //how bright the green channel needs to be to trigger a glow
const float glow_distance = .0025;             //size of the glow
const float overexpose = 50;                  //colors more than full bright bleed into other channels to make stuff whiter
...
void frag(){
    ...
    float ld = light_distance/(sin(timer)/8+0.875);

    float glowval = -(dy / 360.0/* * ratio.y*/);
    glowval = glowval*(ld/1.5);
    glowval += 1;
    ...
    dp = -(dp / 360.0);
    float glowval2 = dp*ld / playerglow_strength;
    glowval2 += 1;
    ...
    glowval3 = glowval3*ld / (playerglow_strength*4);
    glowval3 += 1;
    ...
    vec4 glowcolor = (glow_color*glowval*glow_strength)*blurcolor;//vec4(0,blurcolor,0,0);

    outcolor = outcolor + glowcolor;//outcolor*.5+basecolor;
