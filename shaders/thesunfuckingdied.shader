...
const vec4  light_color = vec4(1, 0.5, 0.33, 1.0); // r, g, b, a. the gradient light that is above (and on top) of the water
const float light_distance = 0.3;                    // less = farther distance
const vec4  glow_color = vec4(1.0, 0.5, 0.33, 0.25);  // alpha is unused here. the color that white parts glow when near water
const float glow_strength = 10;                 //how strong the glow effect is
const float glow_threshold = .5;               //how bright the green channel needs to be to trigger a glow
...
void frag()
{
    ...
    vec2 rippleval = vec2(0.0, sin(-timer * 3 + sin(timer / 2 + worldPos.x / 7 + worldPos.x / 11 + worldPos.x / 4) / 6 + worldPos.y / 15.0) * 0.003 * ratio.y);

    vec4 heatwave_color = texture(framebuf, screencoords.xy);
    ...
    dy -= noise(worldPos.x / 32.0 + timer *  2   ) * 4.5;
    dy -= noise(worldPos.x / 16.0 + timer * -4.35) * 9;
    //dy -= noise(worldPos.x / 8.0  + timer *  1   ) * 18;
    dy -= noise(worldPos.x / 72.0  + timer *  -3   ) * 36;
    dy -= noise(worldPos.x / 144.0  + timer *  3   ) * 72;
