...
const vec2  shake_intensity = vec2(1, 1);       // the intensity of the screen shake in the x and y directions
...
void frag()
{
    ...
    float shake_time_value = noise(-timer) * shake_speed;
    float pulse_time_value = noise(-timer) * pulse_speed;

    // shake the screen randomly in various directions
    vec2 screen_offset = vec2(noise(shake_time_value+shake_intensity.x), noise(shake_time_value+shake_intensity.y)) * ratio * 0.15;
    vec2 shaken_screencoords = screencoords.xy + vec2(noise(timer*27), noise(-timer*29))*0.015*level_param;

    // pulse between the standard palette color and the color specifed by pulse_color
    float mix_value = clamp((noise(-timer*10)+noise(timer*16))/2, 0.3, 0.6);
    vec4 outcolor = mix(texture(framebuf, shaken_screencoords), pulse_color, noise(mix_value)*level_param);
