...
varying vec2 worldPos;
    ...
    void frag(){
        ...
        vec2 rippleval = vec2(sin(-timer*1.9*.5+worldPos.y / 9.0) * .0007*3.0 * ratio.x, sin(-timer*1.2*.5+worldPos.x / 8.0) * .0013*3.0 * ratio.y / 10);
        rippleval += vec2(sin(-timer*1.7*.5+worldPos.y / 16.0) * .0005*3.0 * ratio.x, sin(-timer*1.8*.5+worldPos.x / 14.0) * .0008*3.0 * ratio.y / 10);
        rippleval += vec2(sin(-timer*1.4*.5+worldPos.x / 32.0) * .0006*3.0 * ratio.x, sin(-timer*1.5*.5+worldPos.y / 25.0) * .0009*3.0 * ratio.y / 10);
       ...
       // reflectionval += vec2(sin(-timer*20+worldPos.y / 1.0) * .001 * ratio.x, 0.0); //ripple distortion
        ...
        float darkval = ((1+cos(rippleval.x*1000)/20)+(1+cos(rippleval.x*840)/19)+(1+sin(rippleval.y*530)/20))/2;
        outcolor.r /= 1.25*darkval;
        outcolor.g /= 3*darkval;
        outcolor.b /= 3*darkval;
        outcolor.a = 1.0;
