...
const vec4  light_color = vec4(0.57, 0.305, 0.15, 0.8); // r, g, b, a. the gradient light that is above (and on top) of the water
const vec4  glow_color = vec4(0.8, 0.70, 0.2, 0.0);  // alpha is unused here. the color that white parts glow when near water
const float glow_strength = 10;                 //how strong the glow effect is
const float glow_threshold = .4;               //how bright the green channel needs to be to trigger a glow
const float glow_distance = .0035;             //size of the glow
const float overexpose = .3;                  //colors more than full bright bleed into other channels to make stuff whiter
const float light_distance = 0.8;                //less = farther distance
